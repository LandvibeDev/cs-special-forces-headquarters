...
> - 잘 알려진 포트번호는 유명한 프로토콜에 의해서 정해진다 했는데...  
> - 클라이언트 쪽은 주로 랜덤 port 번호 받는다는데,, 상관없나? 브라우저로 웹서버 접속할 때, 브라우저의 포트는 항상 80???? 그럼 여러 브라우저 띄우면?
...
- 소켓 vs. 포트
  - **포트(Port)**: 프로세스를 식별하기 위해, 호스트 내부적으로 **프로세스가 할당받는 고유한 값**
    ...
    - 만약 지정된 포트 번호를 다른 소켓이 사용하고 있다면, bind() API는 에러를 리턴 ❓ 위랑 상반되는거 아닌가? ❓
    - 논리적인 접속장소
...
![라우터의 범위](2021-03-03-06-23-24.png)
  ...
  > 🔍 **종단간의 원리**(end-end principle) (추가필요) ❓  
